      ...
      <form action="midtrans.html" className="relative px-5 mt-[30px]">
          ...
          <div className="grid grid-cols-3 gap-4">
            {OPTIONS_TOPUP.map((item, i) => (
              <button
                ...
                key={`${item + i}`}
                className="group relative theather-card flex flex-col rounded-3xl p-4 gap-[2px] bg-white/10 backdrop-blur-md hover:bg-[#5236FF] has-[:checked]:bg-[#5236FF] transition-all duration-300"
              >
                <input
                  type="radio"
                  name="amount"
        ...
        <div className="relative h-[98px] w-full max-w-[640px]">
          ...
          <button
            ...
            disabled={isPending}
            onClick={handleTopup}
            className="fixed bottom-[30px] w-[calc(100%-40px)] max-w-[600px] rounded-full p-[12px_18px] h-fit bg-white font-bold text-[#010101]"
          >
            Proceed to Topup
          </button>
          {/* End Proceed Topup */}
