...
export default function SelectTheater() {
  ...
  const handleContinue = () => {
    ...
    dispatch(setTicketDetail({ theater: theater }));
    dispatch(setStep({ step: "TIME" }));
  };

  return (
    <div
      id="Content-Container"
      className="relative flex flex-col w-full max-w-[640px] min-h-screen mx-auto 
    bg-[linear-gradient(179.86deg,_#000000_40.82%,_#0E0E24_99.88%)] overflow-x-hidden text-white"
    >
        ...
        <div id="Theaters" className="tab-content flex flex-col gap-4">
          ...
          {detail.theaters.map((item) => (
            <button
              ...
              className={`relative theather-card flex items-center rounded-3xl p-4 gap-2 backdrop-blur-md transition-all duration-300
                ${theater?._id === item._id ? "bg-purple-700" : "bg-white/10"}
              `}
            >
              <input
                type="radio"
