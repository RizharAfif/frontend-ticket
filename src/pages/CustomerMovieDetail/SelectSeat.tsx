...
export default function SelectSeat() {
  ...
  const handleSelectSeat = (seat: string) => {
    ...
    if (selectedSeats.find((val) => val === seat)) {
      const updatedSeats = selectedSeats.filter((val) => val !== seat);
  ...
  const handleContinue = () => {
    ...
    dispatch(setTicketDetail({ seat: selectedSeats }));
    ...
    >
      ...
      <form action="tickets-payment.html" className="relative px-5 mt-5">
        ...
        <div className="flex items-center justify-center mt-[30px] gap-5">
          ...
          <div className="flex items-center gap-[6px]">
            <span className="w-4 h-4 flex shrink-0 rounded-[6px] bg-[#5236FF]" />
            <span className="font-semibold text-xs leading-[18px]">
              Selected
            </span>
          </div>
        </div>
        {/* End Status Choose Seats */}
